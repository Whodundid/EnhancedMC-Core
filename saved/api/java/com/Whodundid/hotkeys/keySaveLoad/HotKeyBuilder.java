...
public class HotKeyBuilder {
	...
	public boolean buildHotKey(String keyName, String categoryIn, String keyDescription, boolean keyEnabled, KeyActionType keyType) {
		try {
			switch (keyType) {
			case MESSAGESENDER: createdKey = new MessageSenderHotKey(keyName, keys, command, keyDescription); break;
			case CONDITIONAL_MESSAGE_ITEMTEST: createdKey = new ItemTestMessageSenderHotKey(keyName, keys, command, testHeldItemId, keyDescription); break;
