...
public class Server implements IConsoleCommand {
	...
	public void runCommand(ETerminal conIn, EArrayList<String> args, boolean runVisually) {
		if (args.isEmpty()) {
			if (runVisually) { conIn.writeln("connect, disconnect", EColors.green); }
			else {
	...
	private void pingServer(ETerminal conIn, EArrayList<String> args, boolean runVisually) {
		if (args.size() > 1 && args.get(1) != null && !args.get(1).isEmpty()) {
			try {
				...
				if (server.isReachable(5000)) { conIn.writeln(server.getHostAddress() + " is online.", EColors.green); }
				else { conIn.writeln(server.getHostAddress() + " connection timed out.", EColors.red); }
			}
