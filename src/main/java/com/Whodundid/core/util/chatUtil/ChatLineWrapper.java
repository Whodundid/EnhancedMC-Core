...
public class ChatLineWrapper {
    ...
    public static List<IChatComponent> makeList(IChatComponent lineIn, int sizeIn, boolean val1, boolean val2) {
        ...
        for (int j = 0; j < list1.size(); j++) {
            ...
            int i1 = mc.fontRendererObj.getStringWidth(s5);
            ...
            if (i + i1 > sizeIn) {
                String s2 = mc.fontRendererObj.trimStringToWidth(s4, sizeIn - i, false);
                String s3 = s2.length() < s4.length() ? s4.substring(s2.length()) : null;

                if (s3 != null && s3.length() > 0) {
                    int l = s2.lastIndexOf(" ");

                    if (l >= 0 && mc.fontRendererObj.getStringWidth(s4.substring(0, l)) > 0) {
                        s2 = s4.substring(0, l);
                ...
                i1 = mc.fontRendererObj.getStringWidth(s2);
